...
- [x] 1.1 Add CNPG managed role definitions for warehouse read consumers (Airflow and Streamlit) in production and dev warehouse cluster manifests.
- [x] 1.2 Add CNPG managed role definition for warehouse admin LOGIN role in production and dev with least-privilege attributes.
...
- [x] 2.1 Add reconciler workload manifests (on-deploy execution plus periodic convergence) for warehouse and warehouse-dev.
...
- [ ] 3.4 Validate pgAdmin admin LOGIN can connect with Key Vault-backed credentials and remains within expected privilege boundaries.
